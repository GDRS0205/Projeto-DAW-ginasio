...
router.get("/", (req, res, next) => {
  try {
    ...
    const rows = db
      .prepare(
        `SELECT id, name, muscle_group, description, media_url
         FROM exercises
         ${where}
...
router.get("/:id", (req, res, next) => {
  try {
    ...
    const row = db
      .prepare(
        "SELECT id, name, muscle_group, description, media_url FROM exercises WHERE id = ?"
      )
      .get(id);
    if (!row) return res.status(404).json({ error: "Exercício não encontrado." });
    res.json(row);
  } catch (e) {
    next(e);
  }
...
router.put("/:id", requireAuth, (req, res, next) => {
  try {
    ...
    const existing = db
      ...
      .get(id) as any | undefined; // 👈 evita o erro de TS
    ...
    const newGroup =
      typeof muscle_group === "string" && allowedGroups.has(muscle_group)
        ? muscle_group
...
router.post("/:id/duplicate", requireAuth, (req, res, next) => {
  try {
    ...
    const newName =
      bodyName.length > 0 ? bodyName : `${existing.name} (cópia)`;
