...
/* ---------- Helpers numéricos ---------- */
const ensureNumber = (n, def = 0) => Number.isFinite(Number(n)) ? Number(n) : def;
const intMin1 = (n) => Math.max(1, Math.floor(ensureNumber(n, 1)));
const nonNeg = (n) => Math.max(0, ensureNumber(n, 0));
...
/* ---------- Helper de ownership ---------- */
function getOwnedWorkout(id, userId) {
    return db_1.db
        .prepare(`SELECT id, user_id, name, muscle_group, created_at, notes, is_favorite
       FROM workouts
...
router.post("/exercises/:itemId/log", auth_1.requireAuth, (req, res, next) => {
    try {
        ...
        const item = db_1.db
            .prepare(`SELECT id, workout_id, sets, reps, weight
        ...
        const info = db_1.db
            .prepare(`INSERT INTO workout_item_logs (item_id, sets, reps, weight)
         VALUES (?, ?, ?, ?)`)
            .run(itemId, sets, reps, weight);
        const created = db_1.db
            .prepare(`SELECT id, item_id, ts, sets, reps, weight
...
router.get("/exercises/:itemId/logs", auth_1.requireAuth, (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Sem sessão." });
        ...
        const item = db_1.db
            .prepare(`SELECT wi.id, wi.workout_id, wi.exercise_id
        ...
        const last = db_1.db
         ...
         LIMIT ?`)
            .all(itemId, limit);
        const prRow = db_1.db
            .prepare(`SELECT MAX(weight) AS pr
         FROM workout_item_logs
         WHERE item_id = ?`)
            .get(itemId);
        const pr = prRow?.pr ? Number(prRow.pr) : 0;
        const volRow = db_1.db
            .prepare(`SELECT MAX(sets * reps * weight) AS bestVolume
         FROM workout_item_logs
         WHERE item_id = ?`)
            .get(itemId);
...
router.get("/prs", auth_1.requireAuth, (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Sem sessão." });
...
router.get("/stats", auth_1.requireAuth, (req, res, next) => {
    try {
        ...
        const datesRow = db_1.db
            .prepare(`SELECT MIN(created_at) AS first, MAX(created_at) AS last
         FROM workouts WHERE user_id = ?`)
            .get(userId);
        const firstWorkoutDate = datesRow?.first || null;
        const lastWorkoutDate = datesRow?.last || null;
        const topGroupRow = db_1.db
            .prepare(`SELECT muscle_group, COUNT(*) AS c
         FROM workouts
         WHERE user_id = ?
...
router.put("/exercises/:itemId", auth_1.requireAuth, (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Sem sessão." });
        const itemId = Number(req.params.itemId);
        const item = db_1.db
            .prepare(`SELECT id, workout_id, sets, reps, weight, note
         FROM workout_items WHERE id = ?`)
            .get(itemId);
        if (!item?.id)
            return res.status(404).json({ error: "Item não encontrado." });
        ...
        const note = typeof patch.note === "string"
            ...
            : typeof item.note === "string"
                ? item.note
                : null;
        db_1.db.prepare(`UPDATE workout_items
       SET sets = ?, reps = ?, weight = ?, note = ?
       WHERE id = ?`).run(sets, reps, weight, note, itemId);
        const updated = db_1.db
            .prepare(`SELECT wi.id, wi.workout_id, wi.exercise_id, wi.sets, wi.reps, wi.weight,
                wi.position, wi.note,
...
router.delete("/exercises/:itemId", auth_1.requireAuth, (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Sem sessão." });
        ...
        if (!owner)
            return res.status(404).json({ error: "Item não encontrado." });
        db_1.db.prepare(`DELETE FROM workout_items WHERE id = ?`).run(itemId);
        res.json({ ok: true });
    }
    catch (e) {
        next(e);
...
router.get("/", auth_1.requireAuth, (req, res, next) => {
    try {
        ...
        const rows = db_1.db
            .prepare(`SELECT w.id, w.name, w.muscle_group, w.created_at, w.is_favorite
         FROM workouts w
         ${whereSql}
...
router.post("/", auth_1.requireAuth, (req, res, next) => {
    try {
        ...
        const info = db_1.db
            .prepare(`INSERT INTO workouts (name, muscle_group, user_id)
         VALUES (?, ?, ?)`)
            .run(rawName, mg, userId);
        const created = db_1.db
            .prepare(`SELECT id, name, muscle_group, created_at, is_favorite
         FROM workouts
         WHERE id = ? AND user_id = ?`)
...
router.post("/:id/exercises", auth_1.requireAuth, (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Sem sessão." });
        ...
        const maxPosRow = db_1.db
            .prepare(`SELECT COALESCE(MAX(position), 0) AS maxp
         FROM workout_items
         WHERE workout_id = ?`)
            .get(workoutId);
        const nextPos = (maxPosRow?.maxp || 0) + 1;
        const info = db_1.db
            .prepare(`INSERT INTO workout_items
           (workout_id, exercise_id, sets, reps, weight, position)
         VALUES (?, ?, ?, ?, ?, ?)`)
            .run(workoutId, Number(exercise_id), intMin1(sets), intMin1(reps), nonNeg(weight), nextPos);
        const created = db_1.db
            .prepare(`SELECT wi.id, wi.workout_id, wi.exercise_id, wi.sets, wi.reps, wi.weight,
                wi.position, wi.note,
                e.name AS name, e.muscle_group AS muscle_group
...
router.put("/:id/reorder", auth_1.requireAuth, (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Sem sessão." });
        ...
        const tx = db_1.db.transaction((ids) => {
            ids.forEach((itemId, idx) => {
                db_1.db.prepare(`UPDATE workout_items
           SET position = ?
           WHERE id = ?`).run(idx + 1, itemId);
            });
        });
        tx(itemIds);
        res.json({ ok: true });
...
router.post("/:id/duplicate", auth_1.requireAuth, (req, res, next) => {
    try {
        ...
        name = name.trim();
        const tx = db_1.db.transaction((newName) => {
            const rW = db_1.db
                .prepare(`INSERT INTO workouts (name, muscle_group, user_id, is_favorite)
           VALUES (?, ?, ?, ?)`)
                .run(newName, src.muscle_group, userId, src.is_favorite ?? 0);
            const newId = Number(rW.lastInsertRowid);
            const items = db_1.db
                .prepare(`SELECT exercise_id, sets, reps, weight, position, note
           FROM workout_items
           WHERE workout_id = ?
           ORDER BY position ASC, id ASC`)
                .all(srcId);
            if (items.length) {
                const insI = db_1.db.prepare(`INSERT INTO workout_items
             (workout_id, exercise_id, sets, reps, weight, position, note)
           VALUES (?, ?, ?, ?, ?, ?, ?)`);
                for (const it of items) {
                    insI.run(newId, it.exercise_id, it.sets, it.reps, it.weight, it.position, it.note ?? null);
                }
            }
            return newId;
        });
...
router.put("/:id/note", auth_1.requireAuth, (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Sem sessão." });
        ...
        db_1.db.prepare(`UPDATE workouts SET notes = ? WHERE id = ? AND user_id = ?`).run(note, id, userId);
        res.json({ ok: true });
    }
    catch (e) {
        next(e);
...
router.get("/:id/csv", auth_1.requireAuth, (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Sem sessão." });
        ...
        if (!w)
            return res.status(404).json({ error: "Treino não encontrado." });
        const rows = db_1.db
            .prepare(`SELECT wi.position, e.name AS exercise_name, e.muscle_group,
        ...
        res.setHeader("Content-Disposition", `attachment; filename="treino-${id}.csv"`);
        res.send(csv);
    }
    catch (e) {
        next(e);
...
router.get("/:id", auth_1.requireAuth, (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Sem sessão." });
...
router.put("/:id", auth_1.requireAuth, (req, res, next) => {
    try {
        ...
        const mg = typeof rawGroup === "string" && allowed.has(rawGroup)
            ? rawGroup
            : undefined;
        if (!name && !mg)
        ...
        db_1.db.prepare(`UPDATE workouts
       SET ${fields.join(", ")}
...
router.put("/:id/favorite", auth_1.requireAuth, (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Sem sessão." });
        ...
        db_1.db.prepare(`UPDATE workouts
       SET is_favorite = ?
       WHERE id = ? AND user_id = ?`).run(newVal, id, userId);
        const updated = db_1.db
            .prepare(`SELECT id, name, muscle_group, created_at, is_favorite
         FROM workouts
         WHERE id = ? AND user_id = ?`)
